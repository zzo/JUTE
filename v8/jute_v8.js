...
var  fs         = require('fs')
    ...
    ,url        = require('url')
    ,xmlhttp    = require("/home/y/share/node/xml_node.js")
    ,TEST_ROOT  = '$(TEST_ROOT)'
...
function tests_done(data, report_data, cover_object, cover_out) {
    ...
    if (cover_object) {
        ...
        coverage = spawn('/home/y/bin/java', [ '-jar', '/home/y/bin/yuitest-coverage-report.jar', '--format', 'lcov', '-o', dirname + 'lcov-report', cover_out_file ]);
...
//        filter: 'DEBUG',
        ...
        function getScript(tag, cb) {
            ...
            if (src.indexOf('jute.js') > 0) {
                src = '/home/y/share/node/jute_mini.js';
            }
            ...
            if (src) {
                if (src.match(/^http:/)) {
                    ...
                    request.on('response', function (response) {
                        ...
                        response.on('end', function() {
                            ...
                            if (response.statusCode > 299) {
                                DEBUG("ERROR FETCHING: -" + path +'- ' + BASE);
                                // Try it locally
                                path = path.replace(/^\/3.\d+\.\d+\/build\//, BASE);
                                DEBUG("PATH NOW: -" + path + '-');
                    ...
                    if (ssrc[1] === 'coverage=1' && DO_COVERAGE) {
                        ...
                        coverage = spawn('/home/y/bin/java', [ '-jar', '/home/y/bin/yuitest-coverage.jar', '-o', full_path_file, ssrc[0] ]);
                        coverage.on('exit', function(code) {
